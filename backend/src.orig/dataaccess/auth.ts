...
export async function revokeQrCodeRefreshToken(this: DatabaseAccess, account: string, token: string) {
  ...
  await this.knex('token').where('user', user.id).andWhere('isQrToken', true).del();
}
