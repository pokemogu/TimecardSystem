...
const accessTokens: { token: string, expiration: Date }[] = [];
const getUserFromTokenHeader = (header?: string) => {
  ...
  const extractedAccessToken = matches[1];

  // トークン認証
  const token = accessTokens.find((accessToken) => accessToken.token === extractedAccessToken);
  if (!token || !token.expiration || token.expiration < new Date) {
    return undefined;
  }
...
export const handlers = [
  ...
  rest.post<apiif.IssueTokenRequestBody, PathParams, apiif.IssueTokenResponseData | apiif.MessageOnlyResponseBody>('/api/token/issue', (req, res, ctx) => {
  ...
  rest.get<DefaultBodyType, apiif.UserRequestPathParams, apiif.UserInfoResponseBody>('/api/user/:userId', (req, res, ctx) => {
  ...
  rest.get<DefaultBodyType, PathParams, apiif.DevicesResponseBody>('/api/devices', (req, res, ctx) => {
  ...
  rest.get<DefaultBodyType, PathParams, apiif.DepartmentResponseBody>('/api/department', (req, res, ctx) => {
  ...
  rest.get<DefaultBodyType, apiif.optionsApplyRequestPathParams, apiif.ApplyOptionsResponseBody>('/api/options/apply/:type', (req, res, ctx) => {
  ...
  rest.get<DefaultBodyType, PathParams, apiif.ApplyResponseBody>('/api/apply', (req, res, ctx) => {
