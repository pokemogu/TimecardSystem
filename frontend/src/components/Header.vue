...
<template>
  <nav class="navbar navbar-expand-lg navbar-dark bg-dark">
    <div class="container-fluid">
      ...
      <button v-if="props.customButton1" v-on:click="$emit('customButton1')" class="ms-2 btn btn-warning btn-sm"
        role="button">{{ props.customButton1 }}</button>
      <button v-if="props.customButton2" v-on:click="$emit('customButton2')" class="ms-2 btn btn-warning btn-sm"
        role="button">{{ props.customButton2 }}</button>
      <button v-if="props.customButton3" v-on:click="$emit('customButton3')" class="ms-2 btn btn-warning btn-sm"
        role="button">{{ props.customButton3 }}</button>
      <RouterLink v-if="props.isAuthorized" :to="{ name: 'logout' }" class="ms-2 btn btn-warning btn-sm" role="button">
        ログアウト</RouterLink>
      <!--
      <RouterLink
