...
describe('ApplyForm', () => {
  ...
  it('with minimal props', () => {
    ...
    for (const eachTestData of testData) {
      store.$patch({ userDepartment: eachTestData.userDepartment });
      const wrapper1 = mount(ApplyForm, {
        props: {
      ...
      expect(wrapper1.text()).toContain(eachTestData.applyName);
      expect(wrapper1.text()).toContain(eachTestData.userDepartment);
